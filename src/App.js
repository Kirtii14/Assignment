...
const QuestionMarkIcon = () => {
  return (
    // Main SVG element defines the 24x24 space
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* ADDED: Circle element for the border */}
      <circle
        cx="12" // Center X
        cy="12" // Center Y
        r="11.25" // Radius (12 - half of strokeWidth)
        fill="none" // No fill, just border
        stroke="url(#questionMarkGradientInfo)" // Apply gradient to stroke
        strokeWidth="1.5" // Border thickness
      />
      {/* Existing Path elements for the question mark */}
      <path
        d="M9 10C9 8.34315 10.3431 7 12 7C13.6569 7 15 8.34315 15 10C15 11.38 14.1 12.6 13 13.15V14"
        stroke="url(#questionMarkGradientInfo)" // Use same gradient
        strokeWidth="2" // Keep original stroke width for the icon itself
        strokeLinecap="round"
...
const MotionNavbar = ({ style }) => { // ADDED: style prop
  ...
  // MODIFIED: Base style for the tab buttons - Added background properties for hover animation
  const tabBaseStyle = {
    ...
    justifyContent: 'center',
    cursor: 'pointer',
    // MODIFIED: Slowed down transition
    ...
    // ADDED: Background properties for hover animation
    backgroundImage: 'linear-gradient(90deg, rgba(255, 255, 255, 0.05) 0%, rgba(255, 255, 255, 0.05) 100%)',
    backgroundSize: '0% 100%', // Start with 0 width
    backgroundRepeat: 'no-repeat',
    backgroundPosition: 'left center',
    // END ADDED Background properties
    boxShadow: 'none',
    position: 'relative',
    zIndex: 2,
    textAlign: 'center',
  ...
  const activeTextStyle = {
    ...
    backgroundImage: 'none', // Ensure active tab doesn't show hover background
  };

  // ADDED: Style specific to the hover animation trigger
  const tabHoverSpecificStyle = {
    backgroundSize: '100% 100%', // Animate to full width on hover
  };
    ...
    <div style={navbarStyle} ref={tabsContainerRef}>
      ...
      {tabs.map((tab) => (
        <button
          ...
          style={{
            ...tabBaseStyle,
            // MODIFIED: Apply hover animation style if hovered and not active
            ...(hoveredTab === tab && activeTab !== tab ? tabHoverSpecificStyle : {}),
            // MODIFIED: Apply active text style if active (overrides hover bg)
...
const addImageBtnStyle = {
  ...
  boxShadow: `
    inset 2px 2px 4px rgba(0, 0, 0, 0.25),            /* inner dark on bottom-right */
    inset -2px -2px 4px rgba(255, 255, 255, 0.10),    /* inner light on top-left */
    8px 8px 16px rgba(0, 0, 0, 0.45),                 /* outer dark shadow bottom-right */
    -8px -8px 16px rgba(255, 255, 255, 0.06),         /* outer light highlight top-left */
    0px 0px 25px rgba(150, 190, 231, 0.25)            /* soft bluish ambient glow */
  `,
};
